...
const CourseAttendanceDashboardPage: React.FC = () => {
  ...
  const { matrix: filteredData, students: filteredStudents } = useMemo(
    () => filterMatrixBySearch(matrixData, matrixStudents, searchTerm),
    [matrixData, matrixStudents, searchTerm],
  );
  ...
  const loading =
    getCourse.isLoading ||
    getCourseSummary.isLoading ||
    (effectiveDateRange !== "all" && getMonthlyAttendance.isLoading);
  const error =
    getCourse.isError || getCourseSummary.isError || getMonthlyAttendance.isError;
  const summary = getCourseSummary.data ?? null;
  const viewingAllMonths = effectiveDateRange === "all";
