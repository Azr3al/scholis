...
**Surface:** Web FE Leads + new Issues tracker; BE `app_crm` extensions + new `app_issues`  
...
| Architecture | Approach 1: shared kanban kit + separate domain apps (`app_crm` + `app_issues`) |
...
FE                              BE
────────────────────────────    ────────────────────────────────
components/kanban/              app_crm (Lead + observers/mentions)
  KanbanBoard / Column / Card     └─ move_lead → shared notify
                                  └─ comments + mentions → observers
components/board-detail/        app_issues (Issue, Status, Comment, Event)
  mention composer                └─ move_issue → shared notify
  observers list                  └─ comments + mentions → observers
  timeline shell
                                shared helpers (mention → observer, email)
LeadsView ──► app_crm APIs      Organization.notify_*_observers_on_status_change
IssuesView ──► app_issues APIs
```
...
1. BE: org toggles, shared helpers, Lead observers/mentions, `app_issues` + RBAC + tests.
