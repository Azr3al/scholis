...
## 11. Substitution reserve sessions (addendum)

**Date:** 2026-08-26  
**Scope:** Program settings only — no org flag, no form-designer checkbox, no per-course reserve cap.

### Summary

Session-credit courses may hold **N extra calendar days** beyond `max_sessions` for substitution. Teaching sessions count toward `max_sessions`; reserve days are stored as events with `is_substitution_reserve=True` and are capped by the program.

### Program settings (General → Creation method and subject rules)

| Field | Control | Default | Visible when |
|---|---|---|---|
| `is_substitution_reserve_enabled` | Switch — “Substitution reserve days” | `false` | `is_session_credit_scheduling === true` |
| `default_substitution_reserve_days` | Number — “Default reserve days” (0–10) | `0` | reserve switch on |

- `autosave: false` — saved with **Save general settings**.
- Turning session-credit off clears reserve on save (BE normalizes).
- Reserve switch on with days `0` ⇒ cap 0 (staff must set days ≥ 1 to allow reserve picks).
- Changing `default_substitution_reserve_days` affects **new** reserve picks only; existing courses keep saved reserve events until staff removes them.

### Data

**`Program`:** `is_substitution_reserve_enabled` (default `false`), `default_substitution_reserve_days` (default `0`, 0–10).

**`Event`:** `is_substitution_reserve` (default `false`).

Canonical cap helper: `substitution_reserve_cap(program)` — 0 unless session-credit + reserve enabled + positive days.

### Validation

- `len(teaching events) <= effective_max` (unchanged).
- `len(reserve events) <= substitution_reserve_cap(program)`.
- Reject reserve events when cap is 0.
- Course span (`start_date` / `end_date`) still from **all** events (teaching + reserve).

### Create / Schedule calendar

- After `max_sessions` teaching picks are filled, further clicks add reserve picks (up to cap).
- Chip labels: **Session** vs **Substitution reserve**.
- Create submit requires teaching count === `max_sessions`; reserve optional up to cap.
- FE helper: `substitutionReserveCap(program)` in `session-credit-draft.ts`.

### Collision & teacher picker

- Busy checks exclude **effectively ended** courses (query-time).
- `busyReason`: `substitution_reserve` when all overlapping events are reserve; else `schedule_conflict`.
- Teacher picker: soft warning badge **Substitution reserve** vs **Busy** (still selectable).

### Attendance

- Reserve events excluded from attendance marking bootstrap.

### Testing anchors

- Program reserve off or days `0` → reserve events rejected (400).
- Cap `2` → 8 teaching + 2 reserve OK; 8 + 3 reserve → 400.
- Ended course reserve overlap → teacher free; active reserve overlap → `substitution_reserve` badge.
- Bootstrap excludes `is_substitution_reserve=True` events.
