...
**Architecture:** Keep Leads in `app_crm`; add tenant app `app_issues`. Shared BE helpers in `app_utils/board_observers.py`. FE: `components/kanban/` + `components/board-detail/` used by Leads and Issues domain wrappers. Students never receive `issue.*` / `lead.*` grants.
...
| `app_crm/models.py` | `Lead.observers` M2M; `LeadComment.mentions`; `LeadEvent.OBSERVER_ADDED` |
| `app_crm/migrations/…` | Schema + optional permission-independent data |
| `app_crm/services.py` | Create adds observer; `move_lead` calls email helper |
| `app_crm/views.py` | Comments accept mentions; observer endpoints; mention-candidates |
| `app_crm/serializers.py` | Expand observers; comment mentions |
| `app_crm/urls.py` | New routes |
| `app_rbac/catalog.py` + `defaults.py` | `issue.*` permissions + role matrix |
| `app_issues/` | New app: models, serializers, services, views, urls, migrations, tests |
| `schedjuice_backend/settings.py` | Register `app_issues` in `TENANT_APPS` + `INSTALLED_APPS` |
| `schedjuice_backend/urls.py` | `include("app_issues.urls")` |
...
| `src/components/issues/*`, `src/app/(internal)/issues/**` | Issues UI + settings |
...
**Decision locked for implementers:** Keep existing CRM group unchanged; add a new top-level **Issues** item with child Settings (`/issues/settings`, `issue.configure`) — mirrors CRM without a risky nav regroup.

- [ ] **Step 2: Route permissions**

```ts
{ prefix: "/issues/settings", anyOf: ["issue.configure"] },
{ prefix: "/issues", anyOf: ["issue.view"] },
```

Put `/issues/settings` **before** `/issues` if matching is prefix-first.
