"""Remove a student from a course (confirmation required)."""
...
REMOVE_STUDENT_SCHEMA = strict_object_schema(
    properties={
...
def run_remove_student_from_course(
    args: dict[str, Any],
    user: User,
    *,
    ...
    if not student_already_enrolled(course.id, student.id):
    ...
    summary = f"Remove {student.name} from {course.title}?"
...
REMOVE_STUDENT_FROM_COURSE_TOOL = Tool(
    name="remove_student_from_course",
    description=(
        "Remove or unenroll a student from a course roster. "
        "Use when the user wants to remove or drop a student from a course. "
        "Requires course.manage_members. "
        "Always returns pending_confirmation. Provide exactly one of course_id or "
        "course_query, and one of user_id or student_query."
    ),
    parameters=REMOVE_STUDENT_SCHEMA,
    run=run_remove_student_from_course,
    exposure="write",
)
