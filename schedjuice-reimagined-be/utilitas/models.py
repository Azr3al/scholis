...
class PostgresGeneratedColumnQuerySet(models.QuerySet):
    """QuerySet that omits Postgres GENERATED columns from bulk_create inserts."""
    ...
    def _batched_insert(
        ...
        skip = getattr(self.model, "postgres_generated_column_attnames", ())
        if skip:
            fields = [f for f in fields if f.attname not in skip]
        return super()._batched_insert(
            objs,
            fields,
            batch_size,
...
class PostgresGeneratedColumnMixin:
    """
    Exclude ORM fields backed by Postgres GENERATED ALWAYS AS ... STORED columns from writes.

    Django 4.2 still includes nullable generated fields on INSERT (as NULL), which Postgres rejects.
    Subclasses set `postgres_generated_column_attnames` to field attnames to skip.
    """

    postgres_generated_column_attnames: tuple[str, ...] = ()

    def _skip_postgres_generated_columns(self, fields):
        skip = self.postgres_generated_column_attnames
        if not skip:
            return fields
        return [f for f in fields if f.attname not in skip]

    def _do_insert(self, manager, using, fields, returning_fields, raw):
        return super()._do_insert(
            manager,
            using,
            self._skip_postgres_generated_columns(fields),
            returning_fields,
            raw,
        )

    def _do_update(self, base_qs, using, pk_val, values, update_fields, forced_update):
        skip = set(self.postgres_generated_column_attnames)
        if skip:
            values = [row for row in values if row[0].attname not in skip]
...
class BaseModel(models.Model):
    ...
    def get_user_friendly_fields(self, fields: Collection[str]):
        # the 'fields' parameter can contain fields that are not in the 'self.user_friendly_fields' list.
