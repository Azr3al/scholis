...
def _teams_footer() -> str:
    return "<p><em>— SuConnect</em></p>"
...
def _build_teams_text_html(announcement: Announcement) -> str:
    content = _build_announcement_text_content(announcement)
    return f"{_teams_heading(announcement)}{content}{_teams_footer()}"


def _build_teams_message_batches(announcement: Announcement) -> list[tuple[str, list]]:
    ...
    if not image_batches:
        return [(_build_teams_text_html(announcement), [])]

    text_html = _build_teams_text_html(announcement)
