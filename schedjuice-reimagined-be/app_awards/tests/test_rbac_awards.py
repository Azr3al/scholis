...
class AwardBoardApiTests(TestCase):
    ...
    def test_board_grant_includes_display_template(self):
        with schema_context(self.schema_name):
        ...
        display = row["grants"][0]["title"]["display_template"]
        self.assertEqual(display["id"], tmpl.id)
        self.assertTrue(
            any(
    ...
    def test_teacher_display_template_ok_catalog_certificate_forbidden(self):
        with schema_context(self.schema_name):
            ...
            denied = client.get(
                f"{self.api_prefix}/award-titles/{self.top1.id}/certificate"
            )
        self.assertEqual(ok.status_code, 200, ok.content)
        self.assertEqual(ok.json()["data"]["name"], self.top1.name)
