...
/**
 * The single place a domain failure becomes an HTTP status.
 *
 * Lives outside `server/` because status codes are a transport concern and
 * services must stay callable by a worker or a CLI (IMPLEMENTATION.md §3.1).
 * One mapper rather than per-route translation means a new error code cannot
