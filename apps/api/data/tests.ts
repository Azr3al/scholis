...
// No numeric columns here, so the row already is the domain shape. The alias
// exists so services never name a *Row type — the day this table needs real
// mapping, this becomes a mapper and no service changes.
...
// Scoping is a query predicate, not an authz decision. The service decides org
// isolation is required; this just makes the scoped read the easy one to reach
// for.
