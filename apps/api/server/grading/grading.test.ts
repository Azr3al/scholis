...
import { ShortGradingMode, textToRichText } from '@scholis/schema';
...
const seed = async () => {
  ...
  await addQuestion(ctx, {
    ...
    question: {
      ...
      body,
      points: 2,
      settings: { selection: 'single', variant: 'plain', rubric: null },
      options: [
        { body, isCorrect: true },
...
const takeAndSubmit = async (
  ...
  const { attempt, token } = await startAttempt(take, { code, takerName, takerRef: null });
  ...
  return submitAttempt(take, { attemptId: attempt.id, token });
};
...
describe('scoreWrittenAnswer', () => {
  ...
  it('records a mark for a manual short-answer question', async () => {
    const org = await makeOrg(harness.db);
    const user = await makeUser(harness.db, org.id);
    const ctx = authedContext(harness.db, { userId: user.id, orgId: org.id, role: 'teacher' });
    ...
    const shortQ = await addQuestion(ctx, {
      testId: test.id,
      question: {
        type: 'short',
        body,
    ...
    await applyMutations(take, {
      ...
      mutations: [
        ...
        },
        {
          ...
          value: { kind: 'short', doc: textToRichText('My answer') },
          clientSeq: 2,
          at: START.toISOString(),
        },
      ],
    ...
    const submitted = await submitAttempt(take, { attemptId: attempt.id, token });
