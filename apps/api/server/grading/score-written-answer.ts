...
import { emptyRichText, isShortManuallyGraded, richTextSchema, type ResponseValue } from '@scholis/schema';
...
// Re-runs score() over the whole attempt rather than adding a delta.
// Incrementing stays correct until a teacher revises a mark, then drifts
// invisibly.
export const scoreWrittenAnswer = async (
  ...
  const emptyValue: ResponseValue =
    question.type === 'essay'
      ? { kind: 'essay', doc: emptyRichText() }
      : { kind: 'short', doc: emptyRichText() };
