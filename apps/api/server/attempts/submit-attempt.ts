...
// Calls score() from the core directly — there's no autoMark service, because a
// service that only forwards to a pure function is the first brick in a tangle.
//
// Idempotent: the outbox flushes before submit, so a second submit is expected
